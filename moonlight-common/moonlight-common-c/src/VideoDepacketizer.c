...
static void dropFrameState(void) {
    ...
    if (decodingFrame) {
        // Если мы были в процессе сборки — просто сбрасываем флаг и идем дальше.
        // Это безопасно, так как ниже вызывается cleanupFrameState().
        decodingFrame = 0;
    }
...
void processRtpPayload(PNV_VIDEO_PACKET videoPacket, int length,
    ...
    // Вместо LC_ASSERT(firstPacket ^ decodingFrame);
    if (firstPacket && decodingFrame) {
        Limelog("Mid-frame restart detected, dropping partial frame\n");
        dropFrameState(); // Очищаем то, что начали собирать
    }
