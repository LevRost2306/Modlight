...
void cleanupPlatform(void) {
    ...
    // Вместо LC_ASSERT, который убивает процесс:
    if (activeThreads != 0 || activeMutexes != 0 || activeEvents != 0) {
        // Просто пишем в консоль, что ресурсы еще заняты.
        // Это нормально для iPad 2 при быстром перезапуске.
        printf("Platform cleanup: Resources still active (T:%d, M:%d, E:%d). Resetting anyway...\n",
               activeThreads, activeMutexes, activeEvents);
        
        // Опционально: можно принудительно обнулить счетчики,
        // чтобы следующая сессия началась с чистого листа.
        activeThreads = 0;
        activeMutexes = 0;
        activeEvents = 0;
    }
