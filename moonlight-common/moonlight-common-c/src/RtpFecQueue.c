...
static int queuePacket(PRTP_FEC_QUEUE queue, PRTPFEC_QUEUE_ENTRY newEntry, int head, PRTP_PACKET packet, int length, int isParity) {
    ...
    if (isBefore16(packet->sequenceNumber, queue->nextContiguousSequenceNumber)) {
        // Если пришел старый пакет (например, от прошлой сессии до поворота экрана)
        // Мы его просто игнорируем, а не убиваем приложение
        return 0;
    }
...
static int reconstructFrame(PRTP_FEC_QUEUE queue) {
    ...
    if (ret != 0) {
        Limelog("FEC: Reconstruction failed with code %d - ignoring frame\n", ret);
        goto cleanup_packets; // Просто уходим на очистку и ждем следующий кадр
    }
    ...
    for (i = 0; i < totalPackets; i++) {
        if (marks[i]) {
            ...
            if (ret == 0 && i < queue->bufferDataPackets) {
                if (queue->bufferHead == NULL || queue->bufferHead->packet == NULL) {
                    Limelog("FEC Cleanup: queue->bufferHead is NULL, skipping packet %d\n", i);
                    free(packets[i]);
                    continue;
                }
