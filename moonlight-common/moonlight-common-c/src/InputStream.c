...
typedef struct _PACKET_HOLDER {
    ...
    union {
        ...
        unsigned char rawPacket[MAX_INPUT_PACKET_SIZE];  // ← ДОБАВИТЬ
    } packet;
...
// Send UTF-8 text directly to the host, independent of keyboard layout.
// Each Unicode code point is sent as a separate packet.
int LiSendUtf8Text(const char* text) {
    PPACKET_HOLDER holder;
    int err;
    int textLen;
    int i;
    ...
    // Send each UTF-8 code point as a separate packet,
    // matching the behavior of the new moonlight-common-c.
    i = 0;
    while (i < textLen) {
        int cpLen;
        unsigned char firstByte = (unsigned char)text[i];

        // Determine UTF-8 code point length
        if ((firstByte & 0x80) == 0x00)      cpLen = 1;
        else if ((firstByte & 0xE0) == 0xC0) cpLen = 2;
        else if ((firstByte & 0xF0) == 0xE0) cpLen = 3;
        else if ((firstByte & 0xF8) == 0xF0) cpLen = 4;
        else { i++; continue; } // Invalid byte, skip

        // Safety: don't read past end of string
        if (i + cpLen > textLen) break;
        ...
        // New-format packet layout (compatible with old encryption path):
        // [4 bytes] size  = BE32(sizeof(magic) + cpLen) = htonl(4 + cpLen)
        // [4 bytes] magic = UTF8_TEXT_EVENT_MAGIC (0x17, stored as-is on LE)
        // [cpLen bytes] UTF-8 code point
        //
        // Note: The "size" field at offset 0 occupies the same position as
        // the old "packetType" field. Sunshine parses based on the magic
        // value at offset 4, so both old and new format packets are handled
        // correctly.

        sizeField = htonl(4 + cpLen);
        magicField = UTF8_TEXT_EVENT_MAGIC; // LE on ARM/iOS

        memcpy(&holder->packet.rawPacket[0], &sizeField, 4);
        memcpy(&holder->packet.rawPacket[4], &magicField, 4);
        memcpy(&holder->packet.rawPacket[8], &text[i], cpLen);

        holder->packetLength = 8 + cpLen;
...
int LiSendTouchEvent(uint8_t eventType, uint32_t pointerId, float x, float y,
    ...
    pkt = holder->packet.rawPacket;

    // Формат пакета (36 байт):
    // [4] size   = BE32(32)  (36 - 4 = 32)
    // [4] magic  = 0x55000002 (LE на ARM)
    // [1] eventType
    // [1] zero
    // [2] rotation (LE)
    // [4] pointerId (LE)
    // [4] x (float LE)
    // [4] y (float LE)
    // [4] pressureOrDistance (float LE)
    // [4] contactAreaMajor (float LE)
    // [4] contactAreaMinor (float LE)

    uint32_t sizeField = htonl(32);
    uint32_t magicField = SS_TOUCH_MAGIC;  // LE на ARM/iOS
    uint16_t rotLE = rotation;
    uint32_t pidLE = pointerId;

    int off = 0;
    memcpy(&pkt[off], &sizeField, 4);    off += 4;
    memcpy(&pkt[off], &magicField, 4);   off += 4;
    pkt[off++] = eventType;
    pkt[off++] = 0;
    memcpy(&pkt[off], &rotLE, 2);        off += 2;
    memcpy(&pkt[off], &pidLE, 4);        off += 4;
    memcpy(&pkt[off], &x, 4);           off += 4;
    memcpy(&pkt[off], &y, 4);           off += 4;
    memcpy(&pkt[off], &pressureOrDistance, 4); off += 4;
    memcpy(&pkt[off], &contactAreaMajor, 4);   off += 4;
    memcpy(&pkt[off], &contactAreaMinor, 4);   off += 4;

    holder->packetLength = off; // 36
